import db from "@/db/drizzle";
import { lessons } from "@/db/schema";
import { getIsAdmin } from "@/lib/admin";
import { eq } from "drizzle-orm";
import { NextResponse, NextRequest } from "next/server";

export const GET = async (req: NextRequest, props: { params: Promise<{ lessonId: number }> }) => {
  const { lessonId } = await props.params;

  if(!getIsAdmin()) {
    return new NextResponse("Unhautorized", { status: 403 });
  }

  const data = await db.query.lessons.findFirst({
    where: eq(lessons.id, lessonId),
  });

  return NextResponse.json(data);
};

export const PUT = async (req: NextRequest, props: { params: Promise<{ lessonId: number }> }) => {
  const { lessonId } = await props.params;

  if(!getIsAdmin()) {
    return new NextResponse("Unhautorized", { status: 403 });
  }

  const body = await req.json();
  const data = await db.update(lessons).set({
    ...body,
  }).where(eq(lessons.id, lessonId)).returning();

  return NextResponse.json(data[0]);
};

export const DELETE = async (req: NextRequest, props: { params: Promise<{ lessonId: number }> }) => {
  const { lessonId } = await props.params;

  if(!getIsAdmin()) {
    return new NextResponse("Unhautorized", { status: 403 });
  }

  const data = await db.delete(lessons)
    .where(eq(lessons.id, lessonId)).returning();

  return NextResponse.json(data[0]);
};